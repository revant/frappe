...
def rename_parent_and_child(doctype, old, new, meta):
    # rename the doc
    frappe.db.sql(
        "UPDATE `tab{0}` SET `name`={1} WHERE `name`={1}".format(doctype, "%s"),
        (new, old),
    )
...
def update_autoname_field(doctype, new, meta):
    ...
    if meta.get("autoname"):
        ...
        if field and field[0] == "field":
            frappe.db.sql(
                "UPDATE `tab{0}` SET `{1}`={2} WHERE `name`={2}".format(
                    doctype, field[1], "%s"
                ),
                (new, new),
            )
...
def validate_rename(doctype, new, meta, merge, force, ignore_permissions):
    ...
    exists = frappe.db.sql(
        "select name from `tab{doctype}` where name=%s for update".format(
            doctype=doctype
        ),
        new,
    )
    ...
    if not (force or ignore_permissions) and not meta.allow_rename:
        frappe.msgprint(
            _("{0} not allowed to be renamed").format(_(doctype)), raise_exception=1
        )
...
def update_child_docs(old, new, meta):
    ...
    for df in meta.get_table_fields():
        frappe.db.sql(
            "update `tab%s` set parent=%s where parent=%s" % (df.options, "%s", "%s"),
            (new, old),
        )
