...
class DatabaseQuery(object):
    ...
    def prepare_args(self):
        ...
        if self.grouped_or_conditions:
            self.conditions.append(
                "({0})".format(" or ".join(self.grouped_or_conditions))
            )
    ...
    def sanitize_fields(self):
        ...
        for field in self.fields:
            if sub_query_regex.match(field):
                if any(
                    keyword in field.lower().split() for keyword in blacklisted_keywords
                ):
                    _raise_exception()

                if any(
                    "({0}".format(keyword) in field.lower()
    ...
    def prepare_filter_condition(self, f):
        ...
        if f.operator.lower() in (
            ...
            fallback = "''"
            value = [
                frappe.db.escape((v.name or "").strip(), percent=False) for v in result
            ]
            if len(value):
    ...
    def build_match_conditions(self, as_condition=True):
        ...
        if (
            ...
            if not self.shared:
                frappe.throw(
                    _("No permission to read {0}").format(self.doctype),
            ...
            if role_permissions.get("if_owner", {}).get("read"):
                self.match_conditions.append(
                    "`tab{0}`.`owner` = {1}".format(
                        self.doctype, frappe.db.escape(self.user, percent=False)
                    )
                )
            ...
            if not group_function_without_group_by:
                ...
                if meta.sort_field and "," in meta.sort_field:
                    ...
                    args.order_by = ", ".join(
                        [
                            "`tab{0}`.`{1}` {2}".format(
                                self.doctype, f.split()[0].strip(), f.split()[1].strip()
                            )
...
def get_order_by(doctype, meta):
    ...
    if meta.sort_field and "," in meta.sort_field:
        ...
        order_by = ", ".join(
            [
                "`tab{0}`.`{1}` {2}".format(
                    doctype, f.split()[0].strip(), f.split()[1].strip()
                )
                for f in meta.sort_field.split(",")
            ]
        )
...
def get_date_range(operator, value):
    ...
    timespan = (
        period_map[operator] + " " + timespan_map[value]
