...
class Meta(Document):
    ...
    def as_dict(self, no_nulls=False):
        def serialize(doc):
            ...
            for key in doc.__dict__:
                ...
                if isinstance(
                    value, (frappe.text_type, int, float, datetime, list, tuple)
    ...
    def get_table_fields(self):
        if not hasattr(self, "_table_fields"):
            if self.name != "DocType":
                self._table_fields = self.get(
                    "fields", {"fieldtype": ["in", table_fields]}
                )
            else:
    ...
    def sort_fields(self):
        ...
        if custom_fields:
            ...
            # standard fields
            newlist += [
                df for df in self.get("fields") if not df.get("is_custom_field")
            ]
        ...
        for link in dashboard_links:
            ...
            if not link.added:
                ...
                data.transactions.append(
                    dict(label=link.group, items=[link.link_doctype])
                )
