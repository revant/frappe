...
class Newsletter(WebsiteGenerator):
    def onload(self):
        if self.email_sent:
            self.get("__onload").status_count = (
                dict(
                    frappe.db.sql(
    ...
    def send_emails(self):
        ...
        if self.recipients:
            self.queue_all()
            frappe.msgprint(
                _("Email queued to {0} recipients").format(len(self.recipients))
            )
    ...
    def queue_all(self, test_email=False):
        ...
        args = {"message": self.get_message(), "name": self.name}
        frappe.sendmail(
    ...
    def get_message(self):
        ...
        return {"Rich Text": self.message, "Markdown": markdown(self.message_md)}[
...
def get_email_groups(name):
    return frappe.db.get_all(
        ...
        ["email_group"],
        {"parent": name, "parenttype": "Newsletter"},
    )
...
def subscribe(email, email_group=_("Website")):
    ...
    content = ""
    if email_template:
        args = dict(email=email, confirmation_url=url, email_group=email_group)
    ...
    frappe.sendmail(
        email,
        subject=getattr("email_template", "subject", "") or _("Confirm Your Email"),
        content=content,
        now=True,
    )
