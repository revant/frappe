...
from frappe.chat.util import safe_json_loads, dictify, listify, squashify, get_if_empty
...
class ChatRoom(Document):
    def validate(self):
        if self.is_new():
            users = get_chat_room_user_set(
                self.users, filter_=lambda u: u.user != session.user
            )
            self.update(dict(users=users))
        ...
        if self.type == "Direct":
            ...
            if self.is_new():
                if is_direct(self.owner, other.user, bidirectional=True):
                    frappe.throw(
                        _("Direct room with {0} already exists.").format(other.user)
                    )
        ...
        if not self.is_new():
            ...
            if diff:
                ...
                frappe.publish_realtime(
                    "frappe.chat.room:update", update, room=self.name, after_commit=True
                )
...
def create(kind, token, users=None, name=None):
    ...
    if room:
        ...
        for user in users:
            frappe.publish_realtime(
                "frappe.chat.room:create", room, user=user, after_commit=True
            )
