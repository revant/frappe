...
def send(user, room, content, type="Content"):
    ...
    frappe.publish_realtime(
        "frappe.chat.message:create", mess, room=room, after_commit=True
    )
...
def seen(message, user=None):
    ...
    if has_message:
        ...
        resp = dict(
            message=message,
            data=dict(seen=json.loads(mess._seen) if mess._seen else []),
        )

        frappe.publish_realtime(
            "frappe.chat.message:update", resp, room=room, after_commit=True
        )
...
def history(room, fields=None, limit=10, start=None, end=None):
    ...
    frappe.enqueue(
        "frappe.chat.doctype.chat_message.chat_message.mark_messages_as_seen",
        message_names=[m.name for m in mess],
