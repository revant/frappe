...
class PropertySetter(Document):
    ...
    def validate_fieldtype_change(self):
        if (
            self.field_name in not_allowed_fieldtype_change
            and self.property == "fieldtype"
        ):
            frappe.throw(
    ...
    def get_property_list(self, dt):
        return frappe.db.get_all(
            ...
            filters={
                ...
                "fieldtype": [
                    "not in",
                    ("Section Break", "Column Break", "HTML", "Read Only", "Fold")
    ...
    def get_defaults(self):
        if not self.field_name:
            return frappe.db.sql(
                "select * from `tabDocType` where name=%s", self.doc_type, as_dict=1
            )[0]
        else:
            return frappe.db.sql(
