...
def get_token(*args, **kwargs):
    ...
    try:
        ...
        if not out.error and "openid" in out.scope:
            token_user = frappe.db.get_value(
                "OAuth Bearer Token", out.access_token, "user"
            )
            token_client = frappe.db.get_value(
                "OAuth Bearer Token", out.access_token, "client"
            )
            client_secret = frappe.db.get_value(
                "OAuth Client", token_client, "client_secret"
            )
            ...
            id_token = {
                ...
                "exp": int(
                    (
                        frappe.db.get_value(
                            "OAuth Bearer Token", out.access_token, "expiration_time"
                        )
                ...
                "sub": frappe.db.get_value(
                    "User Social Login",
                    {"parent": token_user, "provider": "frappe"},
                    "userid",
                ),
                "iss": frappe_server_url,
                "at_hash": frappe.oauth.calculate_at_hash(
                    out.access_token, hashlib.sha256
                ),
            }
...
def revoke_token(*args, **kwargs):
    ...
    headers, body, status = get_oauth_server().create_revocation_response(
        r.url, headers=r.headers, body=r.form, http_method=r.method
    )
...
def openid_profile(*args, **kwargs):
    ...
    first_name, last_name, avatar, name = frappe.db.get_value(
        "User", frappe.session.user, ["first_name", "last_name", "user_image", "name"]
    )
    frappe_userid = frappe.db.get_value(
        "User Social Login",
        {"parent": frappe.session.user, "provider": "frappe"},
        "userid",
    )
