...
class TestAutoAssign(unittest.TestCase):
    ...
    def test_load_balancing(self):
        ...
        for user in ("test@example.com", "test1@example.com", "test2@example.com"):
            self.assertEqual(
                len(frappe.get_all("ToDo", dict(owner=user, reference_type="Note"))), 10
            )

        # clear 5 assignments for first user
        # can't do a limit in "delete" since postgres does not support it
        for d in frappe.get_all(
            "ToDo", dict(reference_type="Note", owner="test@example.com"), limit=5
        ):
    ...
    def test_assignment_rule_condition(self):
        ...
        note1_todo = frappe.get_all(
            "ToDo",
            filters=dict(
                reference_type="Note", reference_name=note1.name, status="Open"
            ),
        )[0]
        ...
        note2_todo = frappe.get_all(
            "ToDo",
            filters=dict(
                reference_type="Note", reference_name=note2.name, status="Open"
            ),
            fields=["name", "date"],
        )[0]
        self.assertNotEqual(
