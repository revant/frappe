...
class AssignmentRule(Document):
    def validate(self):
        ...
        if not len(set(assignment_days)) == len(assignment_days):
            ...
            frappe.throw(
                _("Assignment Day {0} has been repeated.").format(
...
def reopen_closed_assignment(doc):
    todo_list = frappe.db.get_all(
        "ToDo",
        filters=dict(
            reference_type=doc.doctype, reference_name=doc.name, status="Closed"
        ),
    )
    if not todo_list:
        return False
...
def get_assignment_rules():
    return [
        ...
        for d in frappe.db.get_all(
            "Assignment Rule", fields=["document_type"], filters=dict(disabled=0)
        )
    ]
