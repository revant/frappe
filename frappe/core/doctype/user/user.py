...
class User(Document):
    ...
    def check_demo(self):
        if frappe.session.user == "demo@erpnext.com":
            frappe.throw(
                _(
                    "Cannot change user details in demo. Please signup for a new account at https://erpnext.com"
                ),
                title=_("Not Allowed"),
            )
...
def reset_otp_secret(user):
    ...
    if frappe.session.user in ["Administrator", user]:
        ...
        email_args = {
            ...
            "message": "<p>Your OTP secret on {} has been reset. If you did not perform this reset and did not request it, please contact your System Administrator immediately.</p>".format(
