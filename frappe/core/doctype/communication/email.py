...
def make(
    ...
    is_error_report = (
        doctype == "User" and name == frappe.session.user and subject == "Error Report"
    )
    ...
    if (
        ...
        raise frappe.PermissionError(
            "You are not allowed to send emails related to: {doctype} {name}".format(
...
def get_recipients_cc_and_bcc(
    doc, recipients, cc, bcc, fetched_from_email_account=False
):
    ...
    if not cc:
        cc = get_cc(
            doc, recipients, fetched_from_email_account=fetched_from_email_account
        )
...
def remove_administrator_from_email_list(email_list):
    administrator_email = list(
        filter(lambda emails: "Administrator" in emails, email_list)
    )
            ...
            if isinstance(a, string_types):
                ...
                try:
                    ...
                    file_id = frappe.db.get_list(
                        "File", or_filters={"file_name": a, "name": a}, limit=1
                    )
                    if not file_id:
...
def mark_email_as_seen(name=None):
    try:
        if (
            name
            and frappe.db.exists("Communication", name)
            and not frappe.db.get_value("Communication", name, "read_by_recipient")
        ):
            frappe.db.set_value("Communication", name, "read_by_recipient", 1)
            frappe.db.set_value("Communication", name, "delivery_status", "Read")
            frappe.db.set_value(
                "Communication", name, "read_by_recipient_on", get_datetime()
            )
            frappe.db.commit()
    except Exception:
