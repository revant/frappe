...
def upload(
    ...
    def filter_empty_columns(columns):
        ...
        if empty_cols:
            if columns[-1 * len(empty_cols) :] == empty_cols:
                ...
                frappe.msgprint(
                    _("Please make sure that there are no empty columns in the file."),
                    raise_exception=1,
                )
    ...
    def make_column_map():
        ...
        for i, d in enumerate(doctype_row[1:]):
            if d not in ("~", "-"):
                ...
                if dt:
                    column_idx_to_fieldname[(dt, parentfield)][i + 1] = rows[
                        row_idx + 2
                    ][i + 1]
                    column_idx_to_fieldtype[(dt, parentfield)][i + 1] = rows[
                        row_idx + 4
                    ][i + 1]
                ...
                if (not doc) or main_doc_empty(rows[idx]):
                    for dt, parentfield in doctypes:
                        ...
                        for column_idx in column_idx_to_fieldname[(dt, parentfield)]:
                            try:
                                fieldname = column_idx_to_fieldname[(dt, parentfield)][
                                    column_idx
                                ]
                                fieldtype = column_idx_to_fieldtype[(dt, parentfield)][
                                    column_idx
                                ]
                                ...
                                if fieldtype in ("Int", "Check"):
                                    ...
                                    d[fieldname] = duration_to_seconds(
                                        cstr(d[fieldname])
                                    )
                                    ...
                                    fieldtype in ("Link", "Dynamic Link", "Data")
                                    and d[fieldname]
                                ):
    ...
    if not rows:
        ...
        if file_extension == ".xlsx" and from_data_import == "Yes":
            ...
            rows = read_xlsx_file_from_attached_file(
                file_url=data_import_doc.import_file
            )
            ...
            try:
                ...
                if parentfield:
                    ...
                    if (
                        overwrite
                        and doc.get("name")
                        and frappe.db.exists(doctype, doc["name"])
                    ):
    ...
    if data_import_doc:
        ...
        if (
            error_flag
            and data_import_doc.skip_errors
            and len(data) != len(data_rows_with_error)
        ):
...
def get_parent_field(doctype, parenttype):
    ...
    if parenttype:
        ...
        if not parentfield:
            frappe.msgprint(
                _("Did not find {0} for {0} ({1})").format(
