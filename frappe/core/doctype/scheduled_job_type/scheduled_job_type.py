...
class ScheduledJobType(Document):
    ...
    def is_job_in_queue(self):
        queued_jobs = get_jobs(site=frappe.local.site, key="job_type")[
            frappe.local.site
        ]
    ...
    def get_next_execution(self):
        CRON_MAP = {
            ...
            "Hourly Long": "0 * * * *",
            "All": "0/"
            + str((frappe.get_conf().scheduler_interval or 240) // 60)
            + " * * * *",
        }
    ...
    def on_trash(self):
        frappe.db.sql(
            "delete from `tabScheduled Job Log` where scheduled_job_type=%s", self.name
        )
