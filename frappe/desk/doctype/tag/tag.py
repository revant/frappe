...
class DocTags:
    ...
    def update(self, dn, tl):
        ...
        try:
            frappe.db.sql(
                "update `tab%s` set _user_tags=%s where name=%s"
                % (self.dt, "%s", "%s"),
                (tags, dn),
            )
...
def update_tags(doc, tags):
    ...
    for tag in new_tags:
        if not frappe.db.exists(
            "Tag Link", {"parenttype": doc.doctype, "parent": doc.name, "tag": tag}
        ):
            frappe.get_doc(
                {
                    "doctype": "Tag Link",
...
def delete_tag_for_document(dt, dn, tag):
    frappe.db.sql(
        """DELETE FROM `tabTag Link` WHERE `document_type`=%s AND `document_name`=%s AND tag=%s""",
        (dt, dn, tag),
    )
