...
def add_custom_doctypes(data, doctype_info):
    ...
    add_section(
        ...
        [
            d
            for d in doctype_info
            if (d.custom and d.document_type in ("Setup", "Master", ""))
        ],
    )
...
def get_links_for_module(app, module):
    return [
        {"value": l.get("name"), "label": l.get("label")}
        for l in get_links(app, module)
    ]
...
def update_hidden_modules(category_map):
    ...
    for category in category_map:
        ...
        if home_settings.get(
            "modules_by_category"
        ) and home_settings.modules_by_category.get(category):
            module_placement = [
                d
...
def get_home_settings():
    ...
    home_settings = frappe.cache().hget(
        "home_settings", frappe.session.user, get_from_db
    )
