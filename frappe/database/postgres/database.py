...
class PostgresDatabase(Database):
    ...
    def updatedb(self, doctype, meta=None):
        ...
        * updates indices
        """
        res = self.sql(
            "select issingle from `tabDocType` where name='{}'".format(doctype)
        )
        if not res:
    ...
    def add_index(self, doctype, fields, index_name=None):
        ...
        self.sql(
            """CREATE INDEX IF NOT EXISTS "{}" ON `{}`("{}")""".format(
                index_name, table_name, '", "'.join(fields)
            )
        )
    ...
    def get_table_columns_description(self, table_name):
        ...
        return self.sql(
			...
			GROUP BY a.column_name, a.data_type, a.column_default, a.character_maximum_length;""".format(
...
def modify_query(query):
    ...
    if re.search("from tab", query, flags=re.IGNORECASE):
        query = re.sub(
            "from tab([a-zA-Z]*)", r'from "tab\1"', query, flags=re.IGNORECASE
        )
...
def replace_locate_with_strpos(query):
    ...
    if re.search(r"locate\(", query, flags=re.IGNORECASE):
        query = re.sub(
            r"locate\(([^,]+),([^)]+)\)", r"strpos(\2, \1)", query, flags=re.IGNORECASE
        )
    return query
