...
def execute():
    ...
    doctypes_with_text_fields = frappe.get_all(
        "DocField", fields=["parent", "fieldname"], filters={"fieldtype": "Text Editor"}
    )
    ...
    for opts in doctypes_with_text_fields:
        ...
        for data in result:
            ...
            if html != old_value:
                ...
                frappe.db.set_value(
                    opts.parent, data.name, opts.fieldname, html, update_modified=False
                )
...
def scrub_relative_urls(html):
    ...
    try:
        return re.sub(
            """src[\s]*=[\s]*['"]files/([^'"]*)['"]""", 'src="/files/\g<1>"', html
        )
...
def print_diff(html, old_value):
    ...
    diff = difflib.unified_diff(
        old_value.splitlines(1), html.splitlines(1), lineterm=""
    )
    print("\n".join(list(diff)))
