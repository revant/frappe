            ...
            if meta.issingle:
                ...
                if password:
                    set_encrypted_password(
                        doctype, doctype, password, fieldname=df.fieldname
                    )
                    frappe.db.set_value(
                        doctype, doctype, df.fieldname, "*" * len(password)
                    )

            else:
                for d in frappe.db.sql(
                    """select name, `{fieldname}` from `tab{doctype}`
					where `{fieldname}` is not null""".format(
                        fieldname=df.fieldname, doctype=doctype
                    ),
                    as_dict=True,
                ):

                    set_encrypted_password(
                        doctype, d.name, d.get(df.fieldname), fieldname=df.fieldname
                    )

                frappe.db.sql(
                    """update `tab{doctype}` set `{fieldname}`=repeat("*", char_length(`{fieldname}`))""".format(
