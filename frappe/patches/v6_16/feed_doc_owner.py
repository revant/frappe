...
def execute():
    ...
    for doctype, name in frappe.db.sql(
        """select distinct reference_doctype, reference_name
        ...
        frappe.db.sql(
			...
			where
				...
				and (reference_owner is null or reference_owner = '')""".format(
                doctype=doctype
            ),
            {"doctype": doctype, "name": name, "owner": owner},
        )
