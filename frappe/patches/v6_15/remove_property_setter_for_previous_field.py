        ...
        if not existing_ps:
            ...
            for fieldname, insert_after in custom_fields.items():
                ...
                if fieldname not in _idx:
                    ...
                    if (
                        previous_field != insert_after
                        and cstr(existing_ps.creation) >= "2015-12-28"
                    ):
                        idx_needs_to_be_fixed = True
                        break
...
def get_sorted_fields(doctype, custom_fields):
    ...
    standard_fields_count = frappe.db.sql(
        """select count(name) from `tabDocField`
		where parent=%s""",
        doctype,
    )[0][0]
