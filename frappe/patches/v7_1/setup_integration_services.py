...
def setup_integration_service(app_details, settings=None):
    ...
    if frappe.db.exists("Integration Service", app_details["service_name"]):
        integration_service = frappe.get_doc(
            "Integration Service", app_details["service_name"]
        )
    else:
                ...
                if "".join(set(cstr(settings.get(d.fieldname)))) == "*":
                    setattr(
                        ...
                        get_decrypted_password(
                            doctype, docname, d.fieldname, raise_exception=True
                        ),
                    )
...
def get_parameters(app_details):
    if app_details["service_name"] == "Razorpay":
        ...
        if (
            frappe.conf.paypal_username
            and frappe.conf.paypal_password
            and frappe.conf.paypal_signature
        ):
            return {
                "settings": {
