...
def execute():
    for ps in frappe.get_all(
        "Property Setter", filters={"property": "_idx"}, fields=["doc_type", "value"]
    ):
        custom_fields = frappe.get_all(
            "Custom Field", filters={"dt": ps.doc_type}, fields=["name", "fieldname"]
        )
        ...
        if custom_fields:
            ...
            for custom_field in custom_fields:
                ...
                frappe.db.set_value(
                    "Custom Field", custom_field.name, "insert_after", prev_fieldname
                )
                frappe.db.set_value(
                    "Custom Field", custom_field.name, "idx", custom_field_idx
                )
