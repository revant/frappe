...
def execute():
    ...
    if comm_records_count > 100000:
        enqueue(
            method=move_data_from_communication_to_activity_log, queue="short", now=True
        )
    else:
...
def move_data_from_communication_to_activity_log():
    ...
    frappe.db.sql(
        """insert into `tabActivity Log` (name, owner, modified, creation, status, communication_date,
