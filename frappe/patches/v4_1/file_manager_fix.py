...
def get_replaced_files():
    ...
    new_files = dict(
        frappe.db.sql(
            "select name, file_name from `tabFile` where file_name not like 'files/%'"
        )
    )
    old_files = dict(
        frappe.db.sql(
            "select name, file_name from `tabFile` where ifnull(content_hash, '')=''"
        )
    )
...
def get_file_name(fname, hash):
    ...
    return "{partial}{suffix}.{extn}".format(
        partial=partial, extn=extn, suffix=hash[:5]
    )
