...
def get_monthly_results(
    goal_doctype, goal_field, date_col, filter_str, aggregation="sum"
):
...
def get_monthly_goal_graph_data(
    ...
    try:
        month_to_value_dict = (
            json.loads(history) if history and "{" in history else None
        )
    except ValueError:
    ...
    if month_to_value_dict is None:
        doc_filter = (
            (goal_doctype_link + " = " + frappe.db.escape(docname))
    ...
    for i in range(0, 12):
        ...
        month_year = (
            getdate(date_value).strftime("%B")
            + ", "
            + getdate(date_value).strftime("%Y")
        )
