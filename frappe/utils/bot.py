...
class ShowNotificationBot(BotParser):
    ...
    def get_reply(self):
        if self.has(
            "whatsup", "what's up", "wassup", "whats up", "notifications", "open tasks"
        ):
            n = get_notifications()
    ...
    def get_reply(self):
        if self.startswith("open", "show open", "list open", "get open"):
            if self.tables:
                ...
                filters = (
                    get_notification_config().get("for_doctype").get(doctype, None)
                )
                if filters:
                    ...
                    return _("Can't identify open {0}. Try something else.").format(
                        doctype
                    )
        ...
        if self.startswith(_("list"), _("show")):
            ...
            if self.tables:
                ...
                return self.format_list(
                    frappe.get_list(self.get_doctype(), fields=fields, filters=filters)
                )
    ...
    def get_reply(self):
        if self.startswith("how many"):
            ...
            if self.tables:
                return str(
                    frappe.db.sql(
                        "select count(*) from `tab{0}`".format(self.get_doctype())
                    )[0][0]
                )
            ...
            if table:
                ...
                if self.tables:
                    ...
                    data = frappe.get_list(
                        self.get_doctype(), filters=filters, or_filters=or_filters
                    )
                    if data:
                        return self.format_list(data)
                    else:
                        return _("Could not find {0} in {1}").format(
                            text, self.get_doctype()
                        )
