...
def reset_doc(doctype):
    ...
    try:
        git_link = "/".join(
            [
                "https://raw.githubusercontent.com/frappe",
                app,
                branch,
                doc_path.split("apps/" + app)[1],
            ]
        )
...
def remove_duplicate_fields(doctype):
    for field in frappe.db.sql(
        ...
        frappe.db.sql(
            "delete from tabDocField where fieldname=%s and parent=%s limit 1",
            (field[0], doctype),
        )
    ...
    for dp in doctype_properties:
        ...
        if dp in local_doc and dp not in original_doc:
            make_property_setter(
                doctype, "", dp, local_doc[dp], doctype_properties[dp], for_doctype=True
            )
...
def make_custom_fields(doctype, local_doc, original_doc):
    ...
    """
    local_fields, original_fields = get_fields_dict(local_doc), get_fields_dict(
        original_doc
    )
...
def setup_perms_for(doctype):
    perms = frappe.get_all(
        "DocPerm", fields="*", filters=dict(parent=doctype), order_by="idx asc"
    )
    # get default perms
    try:
