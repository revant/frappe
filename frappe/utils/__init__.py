...
def extract_email_id(email):
    ...
    if (
        email_id
        and isinstance(email_id, string_types)
        and not isinstance(email_id, text_type)
    ):
...
def random_string(length):
    ...
    return "".join(
        [choice(string.ascii_letters + string.digits) for i in range(length)]
    )
...
def update_progress_bar(txt, i, l):
    if not getattr(frappe.local, "request", None):
        ...
        sys.stdout.write(
            "\r{0}: [{1}] {2}%".format(txt, completion_bar, percent_complete)
        )
        sys.stdout.flush()
