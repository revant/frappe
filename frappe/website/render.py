...
def get_static_file_response():
    ...
    response.mimetype = (
        mimetypes.guess_type(frappe.flags.file_path)[0] or "application/octet-stream"
    )
    return response
...
def build_page(path):
    ...
    if context.source:
        ...
        if path.endswith("min.js"):
            html = frappe.get_jloader().get_source(frappe.get_jenv(), context.template)[
                0
            ]
        else:
...
def add_csrf_token(data):
    if frappe.local.session:
        return data.replace(
            "<!-- csrf_token -->",
            '<script>frappe.csrf_token = "{0}";</script>'.format(
