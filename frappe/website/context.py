...
def build_context(context):
    ...
    if context.doc:
        ...
        context_controller_hooks = (
            frappe.get_hooks("extend_website_page_controller_context") or {}
        )
    ...
    if (
        ...
        and context.title
        and not context.title.startswith(context.title_prefix)
    ):
    ...
    if context.show_sidebar:
        ...
        if context.basepath:
            ...
            sidebar_json_path = get_sidebar_json_path(
                context.basepath, look_for_sidebar_json
            )
    ...
    if not context.sidebar_items:
        ...
        if sidebar_items == None:
            ...
            def add_items(sidebar_items, items):
                for d in items:
                    if d.get("enabled") and (
                        (not d.get("role")) or d.get("role") in roles
                    ):
...
def add_metatags(context):
    ...
    if image:
        tags["og:image"] = tags["twitter:image"] = tags["image"] = frappe.utils.get_url(
            image
        )
