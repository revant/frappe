...
class DataMigrationRun(Document):
    ...
    def begin(self):
        ...
        self.mappings = [
            frappe.get_doc("Data Migration Mapping", m.mapping)
            for m in plan_active_mappings
        ]
    ...
    def get_deleted_local_data(self):
        ...
        data = frappe.get_all(
            "Deleted Document", fields=["name", "data"], filters=filters
        )
    ...
    def _push_insert(self):
        ...
        # update page_start
        self.db_set(
            "current_mapping_start", self.current_mapping_start + mapping.page_length
        )

        if len(data) < mapping.page_length:
            # done, no more new data to insert
            self.db_set(
                {"current_mapping_action": "Update", "current_mapping_start": 0}
            )
    ...
    def _push_update(self):
        ...
        for d in data:
            ...
            doc = mapping.get_mapped_record(doc)
            try:
                response_doc = connection.update(
        ...
        if len(data) < mapping.page_length:
            ...
            self.db_set(
                {"current_mapping_action": "Delete", "current_mapping_start": 0}
            )
            # not done with this mapping
            return False
    ...
    def _push_delete(self):
        ...
        for d in data:
            ...
            try:
                response_doc = connection.delete(
                    mapping.remote_objectname, migration_id_value
                )
